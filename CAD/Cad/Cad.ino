  ...
  if (fona.available())      //any data available from the FONA?
  {
    ...
    *bufPtr = 0;
    ...
    if (1 == sscanf(fonaNotificationBuffer, "+CMTI: " FONA_PREF_SMS_STORAGE ",%d", &slot)) {
      ...
      int comaCounter=0;
      int aux = 0;
      char cleangpsdata[100] = "";
      for (int i=0 ; i<=120 ; i++){  
        if ( smsBuffer[i] == ':' && comaCounter==0){
          aux = 0;
          comaCounter=1;
          while ( smsBuffer[i] != ',' ){
            i++;
        ...
        else if ( smsBuffer[i] == ':' && comaCounter==1){
          aux = 0;
          comaCounter=2;
          while ( smsBuffer[i] != ',' ){
            i++;
        ...
        else if ( smsBuffer[i] == ':' && comaCounter==2){
          aux = 0;
          comaCounter=3;
          while ( smsBuffer[i] != ',' ){
            i++;
        ...
        else if ( smsBuffer[i] == ':' && comaCounter==3){
          aux = 0;
          comaCounter=4;
          while ( smsBuffer[i] != ',' ){
            i++;
      ...
      /*if (!fona.sendSMS(callerIDbuffer, "Ubicacion recibida, en un momento le ayudaremos!")) {
